...
import { askGrok, askGrokStream } from "@/app/utils/llmUtils";
...
export const POST = async (req: Request) => {
    ...
    const model = req?.headers?.get("X-Model") || process.env['GROK_MODEL'] || 'grok-beta';
    ...
    if (isStreaming) {
        try {
            console.log('🔄 [CALLING GROK - STREAMING]', requestId);

            const stream = await askGrokStream(prompt, undefined, isProtectedByAiFw, model);
    ...
    try {
        console.log('🔄 [CALLING GROK]', requestId);

        const asyncResponse = askGrok(prompt, undefined, isProtectedByAiFw, model);
        const aiResponse = await asyncResponse;

        console.log('Grok Response:', JSON.stringify(aiResponse, null, 2));
