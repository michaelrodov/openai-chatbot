...
import { askCohere, askCohereStream } from "@/app/utils/llmUtils";
...
export const POST = async (req: Request) => {
    ...
    const model = req?.headers?.get("X-Model") || process.env['COHERE_MODEL'] || 'command';
    ...
    if (isStreaming) {
        try {
            console.log('🔄 [CALLING COHERE - STREAMING]', requestId);

            const stream = await askCohereStream(prompt, undefined, isProtectedByAiFw, model);
                ...
                async start(controller) {
                    try {
                        ...
                        for await (const chunk of stream) {
                            ...
                            // Cohere streaming format - custom structure we created
                            // chunk.type === 'cohere_chunk' and chunk.text contains the content
                            let content = '';

                            if (chunk.type === 'cohere_chunk' && chunk.text) {
                                content = chunk.text;
                            }
    ...
    try {
        console.log('🔄 [CALLING COHERE]', requestId);

        const asyncResponse = askCohere(prompt, undefined, isProtectedByAiFw, model);
        const aiResponse = await asyncResponse;

        console.log('Cohere Response:', JSON.stringify(aiResponse, null, 2));

        // Cohere response structure: aiResponse.content contains the text
